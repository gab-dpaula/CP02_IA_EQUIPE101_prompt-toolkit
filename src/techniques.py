...
def few_shot(tarefa, input):
    ...
    return prompt_builder.adicionar_exemplos(prompt, tarefa["exemplos_fewshot"])
...
def role_prompting(tarefa, input):
    ...
    contexto_persona = tarefa["persona"]
    return prompt_builder.montar_prompt(contexto_persona + "\n" + tarefa["instrucao"], input, tarefa["formato_output"])